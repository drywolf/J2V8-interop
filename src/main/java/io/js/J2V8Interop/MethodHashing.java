...
public class MethodHashing {
  ...
  public static int methodHash(Executable method) throws Exception {
    ...
    if (method instanceof Method) {
        methodDesc +=getTypeString(((Method)method).getReturnType());
    }
    ...
    int hash32 = boxLongHash(hash);
    // System.out.println(methodDesc + " HASH32 ---> " + hash32);
    return hash32;
  }
  ...
  public static Map getInterfaceHashes(Class intf) {
    ...
    Constructor[] constructors = intf.getDeclaredConstructors();
    ...
    for (int i = 0; i < methods.length; i++) {
      ...
      try {
        int hash = methodHash(method);
        map.put(method.toString(), hash);
      } catch (Exception e) {
      }
    }
    ...
    for (int i = 0; i < constructors.length; i++) {
      ...
      try {
        int hash = methodHash(ctor);
        map.put(ctor.toString(), hash);
      } catch (Exception e) {
      }
    }
   ...
   * support undeploy
   */
  public static int calculateHash(Executable method) {
    ...
    // System.out.println("METH ---> " + methodStr);
    int hash = (int)methodHashes.get(methodStr);
    return hash;
    // return hash;
  }
    private static int boxLongHash(long hash) {
        int hash32 = (int)(hash >> 32 ^ (hash & 0xFFFFFFFFL)) & 0x7FFFFFFF;
